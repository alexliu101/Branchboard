...
export const dbUtils = {
	...
	async getExpiringDecisions(daysAhead = 30) {
		const futureDate = new Date(Date.now() + daysAhead * 24 * 60 * 60 * 1000);
		const now = new Date();
		
		const expiringNodes = await db.branchNodes
			.where('type').equals('decision')
			.and(node => node.decisionDeadline && 
				new Date(node.decisionDeadline) >= now && 
				new Date(node.decisionDeadline) <= futureDate)
			.toArray();
			
		// Enhance with context
		const expiringWithContext = await Promise.all(
			expiringNodes.map(async (node) => {
				let context = '';
				if (node.eventId) {
					const event = await db.events.get(node.eventId);
					context = event ? `Event: ${event.title}` : 'Unknown Event';
				} else if (node.scenarioId) {
					const scenario = await db.scenarios.get(node.scenarioId);
					context = scenario ? `Scenario: ${scenario.title}` : 'Unknown Scenario';
				}
				
				const daysLeft = Math.ceil((new Date(node.decisionDeadline) - now) / (1000 * 60 * 60 * 24));
		...
		return expiringWithContext.sort((a, b) => a.daysLeft - b.daysLeft);
	},
